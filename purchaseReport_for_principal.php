...
<head>
  ...
  <link rel="stylesheet" href="report_style_update.css" media="print">
  <style>
...
<script>
  ...
  if (dateRange === 'today') {
    ...
    if (customMonth) {
      ...
      printContents += '<h3>From: ' + startDate + '</h3>' +
                       '<h3>To: ' + endDate + '</h3>';
    }
  }
...
function getLastWeekEndDate() {
  var currentDate = new Date();
  var currentDayOfWeek = currentDate.getDay();
  var daysToSubtract = (currentDayOfWeek === 0) ? 0 : 7 - currentDayOfWeek;
  currentDate.setDate(currentDate.getDate() - daysToSubtract - 7);
  var day = currentDate.getDate();
  var month = currentDate.getMonth() + 1;
  var year = currentDate.getFullYear();
...
document.documentElement.style.setProperty('--avail-width', screen.availWidth + 'px');
</script>
  ...
  function fetchData() {
    ...
    $.ajax({
      url: 'fetchPurchaseReportForPrincipal.php',
      type: 'POST',
      data: {
