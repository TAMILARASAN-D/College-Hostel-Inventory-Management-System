...
if(!empty($product_names) && !is_null($hostel)){
...
switch ($date_range) {
  ...
  case 'custom':
    ...
    echo '<script>alert("Select Product and hostel")</script>';
}
...
$sql = "SELECT * FROM distribution WHERE date BETWEEN ? AND ?";
if (!is_null($product_names) && !is_null($hostel) && !in_array('All Products', $product_names)) {
...
$table = '<table><thead><tr><th width=60px>Hostel</th><th width=120px>Date</th><th width=300px>Product Name</th><th>Quantity (Kg)</th><th>Price(per Kg)</th><th>Total price(per product)</th><th>Editor</th></tr></thead><tbody>';
$total = 0;
$product_total = 0;
...
$table .= '</tbody><tr><td colspan="5">Grand Total:</td><td>₹' . number_format($total, 2) . '</td></tr></table>';
