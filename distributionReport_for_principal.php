...
<head>
  ...
  <title>Distributed Products Report</title>
...
<button onclick="goHosteluserHome()" id="back">Back</button><br>
  <h1>Distributed Products Report</h1>
  <div class="item">
...
<label for="hostel">&nbsp Select Hostel &nbsp</label>
  <br>
...
<button onclick="printReport()" class="print-button">Print</button><br>
  ...
  // Get the selected product name
  var hostel = $('select[name="hostel"] option:selected').text();
  ...
  printContents = '<h1>Distributed Products Report</h1>' +
...
function getMonthName(month) {
  ...
  <script>
    ...
    $.ajax({
      url: 'fetchDistributionReportForPrincipal.php',
      type: 'POST',
      data: {
  ...
  $('select[name="product_name[]"], select[name="hostel"], select[name="date_range"], input[name="start_date"], input[name="end_date"], input[name="custom_month"]').change(fetchData);
});
